...
{#if data}
   <div class="p-2 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-md">
      {#if data.page.status === "UP"}
          <div class="flex">
             <h2 class="text-green-500 font-bold text-center"><span class="p-2 bg-green-500 rounded-lg"><span> All services are online!</h2>
          </div>
      {/if}
 
      {#if data.page.status === "HASISSUES"}
          <div class="flex">
             <h2 class="text-red-500 font-bold text-center"><span class="p-2 bg-red-500 rounded-lg"><span> Some services are having issues!</h2>
          </div>
      {/if}
